    ...
    <div className='flex flex-wrap gap-2 py-4'>
        {tags.map((tag, index) => (
          <div key={index} className='bg-backgroundSecondary border border-borderColor rounded p-2 hover:shadow-md transition-shadow flex-grow w-[calc(50%-8px)] md:max-w-[calc(25%-8px)] md:min-w-[200px]' >
            <Badge className='text-textSecondary bg-backgroundPrimary '>{tag.name}</Badge>
            <p className='text-gray-500 text-sm mb-2 line-clamp-2 pt-2'>{tag.description}</p>
            <div className='text-textSecondary text-xs'>{tag.count.toLocaleString()} questions</div>
          </div>
        ))}
      </div></div>
  )
